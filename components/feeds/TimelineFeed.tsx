// components/feeds/TimelineFeed.tsx
"use client";
...
export default function TimelineFeed({
  ...
  const getKey = useCallback(
    (
      ...
      if (
        pageIndex > 0 &&
        (!previousPageData || !previousPageData.nextCursor)
      ) {
        return null;
      }
      // 最初のページまたはカーソルがある場合
  ...
  return (
    <div className='space-y-0'>
      ...
      {feedItems &&
        feedItems.length > 0 &&
        feedItems.map((item: FeedItemWithRelations) => {
          // 型注釈を追加
          // 自分のリツイート除外フィルター (必要なら)
          if (
            item.type === FeedType.RETWEET &&
            item.user.id === loggedInUserDbId
          ) {
            return null;
          }
          // カード表示の switch 文
      ...
      <div ref={loadMoreRef} className='h-10 flex justify-center items-center'>
        {(isLoadingMore ||
          (isValidating && feedItems && feedItems.length > 0)) && (
          <Loader2 className='h-6 w-6 animate-spin text-muted-foreground' />
        )}
        {!isLoadingMore &&
          !isValidating &&
