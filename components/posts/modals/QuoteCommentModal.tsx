// components/modals/QuoteCommentModal.tsx
"use client";
...
import { Loader2, ImagePlus, XIcon } from "@/components/Icons";
...
import { QuotedItemPreview } from "@/components/feeds/cards/QuotedItemPreview";
...
export function QuoteCommentModal({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (trimmedComment.length === 0 && !selectedFile) {
      toast({
        title: "投稿エラー",
        description: "コメントを入力するか画像を選択してください。",
        variant: "destructive",
      });
      return;
    }
    if (trimmedComment.length > 280) {
      toast({
        title: "投稿エラー",
        description: "コメントは280文字以内で入力してください。",
        variant: "destructive",
      });
      return;
    }
