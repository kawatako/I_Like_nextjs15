...
export function ProfileHeader({ userProfileData, isCurrentUser, initialFollowStatus }: ProfileHeaderProps) {
  ...
  return (
    <section className="mb-8 flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6 border-b pb-8"> {/* 区切り線と余白追加 */}
      {/* 左側: アバター */}
      <Avatar className="w-24 h-24 sm:w-32 sm:h-32 border text-4xl flex-shrink-0"> {/* サイズ調整、縮小防止 */}
        <AvatarImage src={userProfileData.image ?? undefined} alt={`${username} のプロフィール画像`} />
      ...
      <div className="flex flex-col items-center sm:items-start gap-3 flex-1 w-full">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center w-full gap-2">
          <div className="text-center sm:text-left">
            <h1 className="text-2xl sm:text-3xl font-bold break-all">{userProfileData.name || username}</h1>
          ...
          <div className="flex-shrink-0">
            {isCurrentUser ? (
              ...
              <Button variant="secondary" size="sm" disabled>読み込み中...</Button>
            )}
          </div>
        </div>
        ...
        {/* 下段: 自己紹介 */}
        {userProfileData.bio && (
          <p className="text-sm text-center sm:text-left max-w-prose whitespace-pre-wrap"> {/* 改行を反映 */}
            {userProfileData.bio}
          </p>
        )}
