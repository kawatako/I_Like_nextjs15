...
export function SortableListItem({ list, isCurrentUser }: SortableListItemProps) {
  ...
  return (
    <li ref={setNodeRef} style={style} {...attributes} className="flex items-start gap-2 relative">
      {isCurrentUser && (
        <button {...listeners} className="cursor-grab p-1 text-muted-foreground hover:text-foreground" title="ドラッグして並び替え">
          <GripVertical className="h-5 w-5" />
        </button>
      )}

      <Link href={detailUrl} className="flex-1 block p-4 rounded-lg border bg-card text-card-foreground shadow-sm hover:bg-muted/50 transition-colors">
        ...
        {list.items.length > 0 ? (
          <ol className="list-none space-y-2 text-sm">
            ...
            {list._count.items > list.items.length && (
              <li className="text-xs text-muted-foreground pl-8">…さらに表示</li>
            )}
          </ol>
        ) : (
