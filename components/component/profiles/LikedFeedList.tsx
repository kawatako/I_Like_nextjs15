// components/component/profiles/LikedFeedList.tsx
"use client";

import { useCallback } from 'react';
// ★ 必要な型とアクション、フック、コンポーネントをインポート ★
import type { FeedItemWithRelations, PaginatedResponse, RankingListSnippet } from "@/lib/types"; // RankingListSnippet も念のため
import { getLikedFeedItemsAction, type LikedFeedResultItem } from "@/lib/actions/likeActions"; // アクションと結果の型
import { useInfiniteScroll, type UseInfiniteScrollReturn } from '@/components/hooks/useInfiniteScroll'; // 無限スクロールフック (パス確認)
...
export default function LikedFeedList({
  ...
  // SWRInfinite のキー生成関数
  const getKey = useCallback((pageIndex: number, previousPageData: PaginatedResponse<LikedFeedResultItem> | null): LikedFeedKey | null => {
  ...
  const fetcher = useCallback<BareFetcher<PaginatedResponse<LikedFeedResultItem>>>(
  ...
  const {
    ...
    isValidating,
    mutate,
  } = useInfiniteScroll<PaginatedResponse<LikedFeedResultItem>>(getKey, fetcher, { // ★ ジェネリクス型変更 ★
  ...
  if (error) {
     ...
     return <div className="p-4 text-center text-red-500">いいねしたフィードの読み込みに失敗しました。</div>;
  }
  ...
  if (likedItems && likedItems.length === 0 && isReachingEnd) {
    return <div className='text-center text-muted-foreground py-8'>いいねした投稿やランキングはありません。</div>;
  }

  return (
    <div className="space-y-0">
      {/* ★ likedItems 配列を map する ★ */}
      {likedItems && likedItems.map((item: LikedFeedResultItem) => { // ★ item は { likeId, feedItem } の形 ★
         ...
         switch (feedItem.type) {
           case FeedType.POST:
             return <PostCard key={item.likeId} item={feedItem} loggedInUserDbId={loggedInUserDbId}/>;
           case FeedType.RANKING_UPDATE:
             return <RankingUpdateCard key={item.likeId} item={feedItem} loggedInUserDbId={loggedInUserDbId}/>;
           case FeedType.RETWEET:
             return <RetweetCard key={item.likeId} item={feedItem} loggedInUserDbId={loggedInUserDbId} />;
           case FeedType.QUOTE_RETWEET:
             return <QuoteRetweetCard key={item.likeId} item={feedItem} loggedInUserDbId={loggedInUserDbId}/>;
           default:
             console.warn("Unknown feed item type in LikedFeedList:", feedItem.type);
             return null;
         }
      })}
      {/* 無限スクロールトリガーと状態表示 */}
