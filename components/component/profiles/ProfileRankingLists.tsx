...
import {
  loadMoreProfileRankingsAction,
  updateRankingListOrderAction,
} from "@/lib/actions/rankingActions";
...
export function ProfileRankingLists({
  ...
  const getKey = useCallback(
    (
      pageIndex: number,
      previousPageData: PaginatedResponse<RankingListSnippet> | null
    ): [string, string, ListStatus, string | null] | null => {
      if (pageIndex === 0)
        return ["profileRankings", targetUserId, status, null]; // 初回
  ...
  const fetcher = useCallback(
    async (key: [string, string, ListStatus, string | null]) => {
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      if (!over || !lists || active.id === over.id) {
        return;
      }
      ...
      const orderedIds = arrayMove(lists, oldIndex, newIndex).map(
        (list) => list.id
      );
  ...
  return (
    ...
    <DndContext
      ...
      onDragEnd={isCurrentUser ? handleDragEnd : undefined}
    >
      ...
      <div ref={loadMoreRef} className='h-10 flex justify-center items-center'>
        {(isLoadingMore || (isValidating && lists && lists.length > 0)) && (
          <Loader2 className='h-6 w-6 animate-spin text-muted-foreground' />
        )}
        {!isLoadingMore &&
          !isValidating &&
