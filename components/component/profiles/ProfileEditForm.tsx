...
import { Card, CardHeader, CardTitle, CardDescription, CardContent } from "@/components/ui/card"; // Card関連
...
type InitialProfileData = Pick<
  User,
  'name' | 'bio' | 'location' | 'birthday' | 'image' | 'coverImageUrl' | 'socialLinks' | 'username' // username もリダイレクト用に含める
>;
...
const ProfileUpdateSchema = z.object({
  ...
  location: z.string().trim().max(100, "場所は100文字以内で入力してください。").optional().nullable(),
  birthday: z.string().nullable().optional()
    // refine で日付として有効かチェック (空文字とnullは許可)
    .refine((date) => date === null || date === undefined || date === '' || !isNaN(Date.parse(date)), {
  ...
  // image, coverImageUrl はファイルで扱うため、ここではスキーマに入れない方が扱いやすいかも
...
export default function ProfileEditForm({ initialData }: ProfileEditFormProps) {
  ...
  const [birthday, setBirthday] = useState<string>(
     initialData.birthday ? new Date(initialData.birthday).toISOString().split('T')[0] : ""
  );
  ...
  const handleCoverFileChange = useCallback((file: File | null) => { setCoverImageFile(file); }, []);
  const handleProfileFileChange = useCallback((file: File | null) => { setProfileImageFile(file); }, []);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!validationResult.success) {
       toast({ title: "入力エラー", description: validationResult.error.errors[0].message, variant: "destructive" });
       return;
    }
    ...
    startTransition(async () => {
      try {
        ...
        if (coverImageFile) {
          uploadPromises.push(uploadCoverImage(coverImageFile).then(url => { if (url) coverImageUrl = url; else throw new Error("カバー画像アップロード失敗"); }));
        } else if (coverImageFile === null && initialData.coverImageUrl) {
           coverImageUrl = null; // 画像削除の場合
        }
        if (profileImageFile) {
          uploadPromises.push(uploadProfileImage(profileImageFile).then(url => { if (url) imageUrl = url; else throw new Error("アイコン画像アップロード失敗"); }));
        ...
        toast({ title: "エラー", description: error instanceof Error ? error.message : "更新できませんでした", variant: "destructive" });
      }
    });
  };
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="relative pl-4 sm:pl-6 pt-[-4rem] z-10 w-fit"> {/* 位置調整 */}
         <ImageUploader
            label="プロフィールアイコン"
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-1">
            ...
            <Input id="name" value={name} onChange={(e) => setName(e.target.value)} maxLength={30} disabled={isProcessing} />
          </div>
          <div className="space-y-1">
            <Label htmlFor="bio">自己紹介</Label>
            <Textarea id="bio" value={bio} onChange={(e) => setBio(e.target.value)} maxLength={160} rows={4} disabled={isProcessing} />
          </div>
          <div className="space-y-1">
            <Label htmlFor="location">場所</Label>
            <Input id="location" value={location} onChange={(e) => setLocation(e.target.value)} maxLength={100} disabled={isProcessing} />
          </div>
          <div className="space-y-1">
            <Label htmlFor="birthday">生年月日</Label>
            <Input id="birthday" type="date" value={birthday} onChange={(e) => setBirthday(e.target.value)} disabled={isProcessing} />
          </div>
        </CardContent>
      </Card>
      ...
      {/* --- リンク --- */}
      <Card>
         <CardHeader><CardTitle>リンク</CardTitle></CardHeader>
         <CardContent className="space-y-4">
           <div className="space-y-1">
             <Label htmlFor="website">Webサイト</Label>
             <Input id="website" type="url" placeholder="https://" value={socialLinks.website} onChange={(e) => setSocialLinks({...socialLinks, website: e.target.value})} disabled={isProcessing}/>
           </div>
           <div className="space-y-1">
             <Label htmlFor="x">X (Twitter)</Label>
             <Input id="x" type="url" placeholder="https://x.com/..." value={socialLinks.x} onChange={(e) => setSocialLinks({...socialLinks, x: e.target.value})} disabled={isProcessing}/>
           </div>
           <div className="space-y-1">
             <Label htmlFor="instagram">Instagram</Label>
             <Input id="instagram" type="url" placeholder="https://instagram.com/..." value={socialLinks.instagram} onChange={(e) => setSocialLinks({...socialLinks, instagram: e.target.value})} disabled={isProcessing}/>
           </div>
           <div className="space-y-1">
             <Label htmlFor="tiktok">TikTok</Label>
             <Input id="tiktok" type="url" placeholder="https://tiktok.com/@..." value={socialLinks.tiktok} onChange={(e) => setSocialLinks({...socialLinks, tiktok: e.target.value})} disabled={isProcessing}/>
           </div>
         </CardContent>
      </Card>
      ...
      <div className="flex justify-end gap-2">
        <Button type="button" variant="outline" onClick={() => router.back()} disabled={isProcessing}>キャンセル</Button>
