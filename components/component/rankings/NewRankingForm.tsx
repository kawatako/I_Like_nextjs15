...
import ImageUploader from "./ImageUploader"; // パスを確認
import type { ActionResult } from "@/lib/types";
import TagInput from "./TagInput"; 

// --- Zod スキーマ ---
const subjectAllowedCharsRegex = /^[\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Han}a-zA-Z0-9 ]+$/u;
const SubjectSchema = z.string().trim().min(1, "テーマを入力してください。").max(50, "テーマは50字以内").regex(subjectAllowedCharsRegex, { message: "使用できない文字種" });
const ItemNameSchema = z.string().trim().min(1, "アイテム名は必須です。").max(100, "アイテム名は100字以内");
const DescriptionSchema = z.string().trim().max(500, "説明は500字以内").optional();
...
export function NewRankingForm() {
  ...
  const { uploadImage: uploadItemImage, isLoading: isItemUploading } = useImageUploader(); // アイテム画像アップロード用
  ...
  const handleItemChange = useCallback((clientId: string, field: keyof Omit<EditableItem, 'clientId' | 'id' | 'imageFile' | 'previewUrl' | 'imageUrl'>, value: string | null) => {
    setEditableItems((currentItems) => currentItems.map((item) => item.clientId === clientId ? { ...item, [field]: value } : item ));
  }, []);

  const handleDeleteItem = useCallback((clientId: string) => {
    // アイテム削除時にプレビューURLを破棄
    const itemToDelete = editableItems.find(item => item.clientId === clientId);
    if (itemToDelete?.previewUrl) { URL.revokeObjectURL(itemToDelete.previewUrl); }
    setEditableItems((currentItems) => currentItems.filter((item) => item.clientId !== clientId));
  }, [editableItems]); // editableItems を依存配列に追加
        ...
        editableItems.forEach(item => {
          if (item.imageFile) {
            uploadPromises.push( uploadItemImage(item.imageFile).then(url => { itemImageUrls.push({ clientId: item.clientId, imageUrl: url }); if (!url) uploadSuccess = false; }) );
             ...
             itemImageUrls.push({ clientId: item.clientId, imageUrl: item.imageUrl ?? null }); // 既存の URL があればそれを使い、なければ null
          }
        });
      ...
      <div className='space-y-6'>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>ランキングのテーマや説明、ヘッダー画像を設定します。</CardDescription>
          </CardHeader>
          <CardContent className='space-y-4'>
            ...
            />
            {/* テーマ入力 */}
            <div><Label htmlFor='subject'>テーマ*</Label><Input id='subject' value={subject} onChange={(e) => setSubject(e.target.value)} maxLength={50} required disabled={isSaving} /></div>
            {/* 説明入力 */}
            <div><Label htmlFor='description'>説明(任意)</Label><Textarea id='description' value={description} onChange={(e) => setDescription(e.target.value)} maxLength={500} disabled={isSaving} rows={3} /></div>
            <div>
        ...
        {/* アイテム編集カード */}
        <Card>
          <CardHeader><CardTitle>ランキングアイテム</CardTitle><CardDescription>...</CardDescription></CardHeader>
          <CardContent className='pt-0 space-y-3'>
            {editableItems.length === 0 && <p className='text-muted-foreground text-sm px-3 py-2'></p>}
            ...
            {/* アイテム追加ボタン */}
            {editableItems.length < 10 && ( <div className='pt-3'><Button variant='outline' size='sm' onClick={handleAddItemSlot} disabled={isSaving}>...</Button></div> )}
          </CardContent>
        </Card>
        {/* フォーム全体のエラー表示 */}
        {formError && (
