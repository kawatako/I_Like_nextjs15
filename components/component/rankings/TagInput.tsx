// components/component/rankings/TagInput.tsx
"use client";

import React, { useState, useCallback, KeyboardEvent } from 'react';
import { Input } from "@/components/ui/input";
import { Badge } from "@/components/ui/badge";
import { XIcon } from "@/components/component/Icons"; // 削除アイコン
...
export default function TagInput({
  ...
  const addTag = useCallback((tagToAdd: string) => {
    ...
    if (newTag.length > maxLength) {
      toast({ title: "文字数超過", description: `タグは${maxLength}文字以内で入力してください。`, variant: "destructive" });
      return;
    }
    if (value.length >= maxTags) {
      toast({ title: "上限到達", description: `タグは${maxTags}個まで設定できます。`, variant: "destructive" });
      return;
    }
    if (value.includes(newTag)) {
      toast({ title: "重複エラー", description: `タグ "${newTag}" は既に追加されています。`, variant: "destructive" });
