...
import {
  ...
  AlertDialogTitle,
  AlertDialogTrigger,
} from "@/components/ui/alert-dialog"; // AlertDialog をインポート
...
import ImageUploader from "../common/ImageUploader"; // 画像アップローダー
import {
  PlusIcon,
  TrashIcon,
  GripVertical,
  ...
  XIcon,
  Loader2,
} from "@/components/component/Icons"; // アイコン
import {
  saveRankingListItemsAction,
  deleteRankingListAction,
} from "@/lib/actions/rankingActions"; // Server Actions
...
export function RankingEdit({ rankingList }: RankingListEditViewProps) {
  ...
  const [tags, setTags] = useState<string[]>(
    rankingList.tags
      ? rankingList.tags.map((tagRelation) => tagRelation.name)
      : []
  );
  ...
  const handleDeleteItem = useCallback(
    (clientId: string) => {
      ...
      if (
        ...
        itemToDelete.previewUrl !== itemToDelete.imageUrl
      ) {
      ...
      setEditableItems((currentItems) =>
        currentItems.map((item) => {
          if (item.clientId === clientId) {
            if (item.previewUrl && item.previewUrl !== item.imageUrl) {
              URL.revokeObjectURL(item.previewUrl);
            }
            const newPreviewUrl = file
  ...
  const handleSave = async (status: ListStatus) => {
    startSaveTransition(async () => {
      ...
      try {
        ...
        console.log("Saving edited ranking:", {
          rankingData,
          itemsData,
          status,
        });
  ...
  const handleDeleteConfirm = async () => {
    if (
      !window.confirm(
        `ランキング「${subject}」を本当に削除しますか？この操作は元に戻せません。`
      )
    ) {
      return;
    }
  ...
  // ボタンの disabled 状態
  const isProcessing =
    isSaving || isHeaderUploading || isItemUploading || isDeleting; // 削除中も考慮
