// components/profiles/LikedRankingListItem.tsx
...
export default function LikedRankingListItem({
  list,
  loggedInUserDbId,
}: LikedRankingListItemProps) {
        ...
        {list.items.length > 0 ? (
          <ol className='list-none pl-0 space-y-1 text-sm text-muted-foreground'>
            ...
            {(list._count?.items ?? 0) > list.items.length && (
              <li className='text-xs text-muted-foreground pt-1 pl-7'>
                ...さらに表示
              </li>
            )}
          </ol>
