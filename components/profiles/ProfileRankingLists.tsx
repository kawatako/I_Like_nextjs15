...
export function ProfileRankingLists({
  ...
  const getKey = useCallback(
    (pageIndex: number, prev: PaginatedResponse<RankingListSnippet> | null) => {
      if (pageIndex === 0)
        return ["profileRankings", targetUserId, status, null] as const;
  ...
  const fetcher = useCallback(
    async ([_key, tId, st, cursor]: readonly [
      string,
      string,
      ListStatusLiteral,
      string | null
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      try {
        const result: ActionResult = await updateRankingListOrderAction(
          orderedIds
        );
  ...
  return (
    ...
    >
      ...
      <div ref={loadMoreRef} className='h-10 flex justify-center items-center'>
        {(isLoadingMore || (isValidating && lists.length > 0)) && (
          <Loader2 className='h-6 w-6 animate-spin text-muted-foreground' />
        )}
        {!isLoadingMore &&
          !isValidating &&
