// components/follows/FollowersList.tsx
"use client";
...
export function FollowersList({ targetUserId }: Props) {
  ...
  const loadMore = useCallback(async () => {
    ...
    setIsLoading(true);
    const res = await getPaginatedFollowers({
      targetUserId,
  ...
  return (
    <div>
      {initialLoaded && items.length === 0 && (
        <p className='text-center py-10'>フォロワーはいません</p>
      )}
