...
export async function getProfileRankingsPaginated({
  ...
  console.log(`[RankingQueries/Paginated] Fetching ${status} lists for user ${userId}, limit ${limit}, cursor ${cursor}`);
  ...
  try {
    ...
    console.log(`[getProfileRankingsPaginated] Found ${rankingLists.length} items for this page. Next cursor: ${nextCursor}`);
    ...
    // --- エラーハンドリング ---
    console.error(`[getProfileRankingsPaginated] Error fetching ${status} lists for user ${userId}:`, error);
