...
type FollowActionResult = {
  success: boolean;
  error?: string;
  status?: "following" | "requested" | "not_following" | "error"; // 操作後の状態を返す（任意）
};
 ...
 */
export async function acceptFollowRequestAction(requestId: string): Promise<FollowActionResult> {
  const { userId: loggedInClerkId } = await auth();
  if (!loggedInClerkId) { return { success: false, error: "ログインしてください。" }; }
  const loggedInUserDbId = await getUserDbIdByClerkId(loggedInClerkId);
  if (!loggedInUserDbId) { return { success: false, error: "ユーザーが見つかりません。" }; }

  console.log(`[FollowActions/Accept] User ${loggedInUserDbId} attempting to accept request ${requestId}`);
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const request = await tx.followRequest.findUnique({
        ...
        select: { // フォローレコード作成に必要なIDと、キャッシュ再検証用のユーザー名を取得
          requesterId: true,
          requestedId: true,
      ...
      if (!request) {
        throw new Error("有効なフォローリクエストが見つからないか、承認権限がありません。");
      }
    ...
    if (result.requesterUsername) revalidatePath(`/profile/${result.requesterUsername}`);
    if (result.requestedUsername) revalidatePath(`/profile/${result.requestedUsername}`);
    if (result.requestedUsername) revalidatePath(`/follows/${result.requestedUsername}`);
    if (result.requesterUsername) revalidatePath(`/follows/${result.requesterUsername}`);

    return { success: true };

  } catch (error) {
    console.error(`[FollowActions/Accept] Error accepting request ${requestId}:`, error);
    const message = error instanceof Error ? error.message : "リクエストの承認中にエラーが発生しました。";
 ...
 * @returns FollowActionResult (成功/失敗)
 */
export async function rejectFollowRequestAction(requestId: string): Promise<FollowActionResult> {
  const { userId: loggedInClerkId } = await auth();
  if (!loggedInClerkId) { return { success: false, error: "ログインしてください。" }; }
  const loggedInUserDbId = await getUserDbIdByClerkId(loggedInClerkId);
  if (!loggedInUserDbId) { return { success: false, error: "ユーザーが見つかりません。" }; }

  console.log(`[FollowActions/Reject] User ${loggedInUserDbId} attempting to reject request ${requestId}`);
  ...
  try {
    ...
    if (updateResult.count === 0) {
      console.warn(`[FollowActions/Reject] Request ${requestId} not found, not pending, or permission denied for user ${loggedInUserDbId}. No records updated.`);
      // throw new Error("有効なフォローリクエストが見つからないか、拒否権限がありません。");
      // ↑ エラーにするか、成功とみなすかは要件次第。ここでは成功として扱う。
    }

    console.log(`[FollowActions/Reject] Request ${requestId} marked as rejected (or was already handled).`);

    // --- キャッシュ再検証 ---
    // 自分のフォローリストページ（リクエストタブ）を再検証
    const currentUser = await prisma.user.findUnique({ where: { id: loggedInUserDbId }, select: { username: true } });
    ...
    console.error(`[FollowActions/Reject] Error rejecting request ${requestId}:`, error);
    const message = error instanceof Error ? error.message : "リクエストの拒否中にエラーが発生しました。";
    return { success: false, error: message };
  }
}
...
export async function getFollowStatus(
  ...
  console.log(`[FollowQueries] Checking follow status between viewer ${viewerDbId} and target ${targetUserId}`);
  ...
  if (viewerDbId === targetUserId) {
    return { status: "SELF", targetUserId: targetUser.id, targetUsername: targetUser.username, targetIsPrivate: targetUser.isPrivate };
  }

  // ログインしていない場合 (フォロー/リクエスト不可)
  if (!viewerDbId) {
    return { status: "CANNOT_FOLLOW", targetUserId: targetUser.id, targetUsername: targetUser.username, targetIsPrivate: targetUser.isPrivate };
  }
  ...
  try {
    ...
    if (existingFollow) {
      console.log(`[FollowQueries] Viewer ${viewerDbId} IS FOLLOWING ${targetUserId}`);
      return { status: "FOLLOWING", targetUserId: targetUser.id, targetUsername: targetUser.username, targetIsPrivate: targetUser.isPrivate };
    }
    ...
    if (sentRequest) {
      console.log(`[FollowQueries] Viewer ${viewerDbId} HAS REQUESTED ${targetUserId}`);
      // return { status: "REQUEST_SENT", targetUserId: targetUser.id, targetUsername: targetUser.username, targetIsPrivate: targetUser.isPrivate, followRequestId: sentRequest.id };
       return { status: "REQUEST_SENT", targetUserId: targetUser.id, targetUsername: targetUser.username, targetIsPrivate: targetUser.isPrivate }; // followRequestId は一旦含めない
    }
    ...
    if (receivedRequest) {
      console.log(`[FollowQueries] Viewer ${viewerDbId} HAS RECEIVED REQUEST FROM ${targetUserId}`);
       return { status: "REQUEST_RECEIVED", targetUserId: targetUser.id, targetUsername: targetUser.username, targetIsPrivate: targetUser.isPrivate };
    }
    ...
    const rejectedRequest = await prisma.followRequest.findUnique({
     ...
     if (rejectedRequest) {
       console.log(`[FollowQueries] Viewer ${viewerDbId} WAS REJECTED BY ${targetUserId}`);
       // 拒否された場合の状態を返すか、CANNOT_FOLLOW にするかは要件次第
       // ここでは仮に REQUEST_SENT と同じにするか、専用の状態を作る
       // return { status: "REJECTED_BY_TARGET", ... };
       return { status: "CANNOT_FOLLOW", targetUserId: targetUser.id, targetUsername: targetUser.username, targetIsPrivate: targetUser.isPrivate }; // 仮にフォロー不可とする
     }


    // 上記のいずれでもない場合: フォローもリクエストもしていない
    console.log(`[FollowQueries] Viewer ${viewerDbId} is NOT FOLLOWING and NO PENDING/REJECTED request for ${targetUserId}`);
    return { status: "NOT_FOLLOWING", targetUserId: targetUser.id, targetUsername: targetUser.username, targetIsPrivate: targetUser.isPrivate };

    // TODO: ブロック機能実装時にはブロック状態もチェックする

  } catch (error) {
    console.error(`[FollowQueries] Error checking follow status between ${viewerDbId} and ${targetUserId}:`, error);
    return { status: "CANNOT_FOLLOW", targetUserId: targetUser.id, targetUsername: targetUser.username, targetIsPrivate: targetUser.isPrivate }; // エラー時はフォロー不可とする
  }
}

// --- ★★★ [ACTION] ユーザーをフォローする / フォローリクエストを送る ★★★ ---
export async function followUserAction(targetUserDbId: string): Promise<FollowActionResult> {
  const { userId: loggedInClerkId } = await auth();
  if (!loggedInClerkId) { return { success: false, error: "ログインしてください。", status: "error" }; }
  const loggedInUserDbId = await getUserDbIdByClerkId(loggedInClerkId);
  if (!loggedInUserDbId) { return { success: false, error: "ユーザー情報が見つかりません。", status: "error" }; }

  // 自分自身はフォローできない
  if (loggedInUserDbId === targetUserDbId) {
    return { success: false, error: "自分自身をフォローすることはできません。", status: "error" };
  }

  console.log(`[FollowActions/Follow] User ${loggedInUserDbId} attempting to follow/request ${targetUserDbId}`);
  ...
  try {
    ...
    // 2. 既にフォロー済みか確認
    const existingFollow = await prisma.follow.findUnique({
      where: { followerId_followingId: { followerId: loggedInUserDbId, followingId: targetUserDbId } }
    });
    if (existingFollow) {
      console.log(`[FollowActions/Follow] Already following ${targetUserDbId}.`);
    ...
    const existingRequest = await prisma.followRequest.findUnique({
      where: {
        requesterId_requestedId: { requesterId: loggedInUserDbId, requestedId: targetUserDbId },
        status: FollowRequestStatus.PENDING,
      }
    });
    if (existingRequest) {
      console.log(`[FollowActions/Follow] Request already sent to ${targetUserDbId}.`);
    ...
    const rejectedRequest = await prisma.followRequest.findUnique({
     ...
     if (rejectedRequest) {
        console.log(`[FollowActions/Follow] Request was previously rejected by ${targetUserDbId}.`);
       return { success: false, error: "以前にフォローリクエストが拒否されたため、再度リクエストを送信できません。", status: "error" };
     }
    ...
    if (!targetUser.isPrivate) {
      ...
      console.log(`[FollowActions/Follow] User ${loggedInUserDbId} successfully followed (public) ${targetUserDbId}`);
      ...
      const loggedInUser = await prisma.user.findUnique({ where: { id: loggedInUserDbId}, select: { username: true }});
      if(loggedInUser?.username) revalidatePath(`/profile/${loggedInUser.username}`); // 自分のプロフィール
      if(loggedInUser?.username) revalidatePath(`/follows/${loggedInUser.username}`); // 自分のフォローリスト
      ...
      console.log(`[FollowActions/Follow] User ${loggedInUserDbId} successfully sent follow request to (private) ${targetUserDbId}`);
    ...
    console.error(`[FollowActions/Follow] Error following/requesting user ${targetUserDbId}:`, error);
    const message = error instanceof Error ? error.message : "フォロー処理中にエラーが発生しました。";
...
export async function unfollowUserAction(targetUserDbId: string): Promise<FollowActionResult> {
  const { userId: loggedInClerkId } = await auth();
  if (!loggedInClerkId) { return { success: false, error: "ログインしてください。", status: "error" }; }
  const loggedInUserDbId = await getUserDbIdByClerkId(loggedInClerkId);
  if (!loggedInUserDbId) { return { success: false, error: "ユーザー情報が見つかりません。", status: "error" }; }
   ...
   // 自分自身はアンフォローできない
  if (loggedInUserDbId === targetUserDbId) {
    return { success: false, error: "自分自身のフォローを解除することはできません。", status: "error" };
  }

  console.log(`[FollowActions/Unfollow] User ${loggedInUserDbId} attempting to unfollow ${targetUserDbId}`);

  try {
    // 該当する Follow レコードを削除
    const deleteResult = await prisma.follow.deleteMany({ // deleteMany の方が where で指定しやすい
      where: {
        followerId: loggedInUserDbId,
        followingId: targetUserDbId,
      }
    ...
    if (deleteResult.count > 0) {
      console.log(`[FollowActions/Unfollow] User ${loggedInUserDbId} successfully unfollowed ${targetUserDbId}`);
    } else {
      // 既にフォローしていなかった場合も、エラーとはせず成功扱いにする（UI上はフォロー解除の状態になるため）
      console.log(`[FollowActions/Unfollow] User ${loggedInUserDbId} was not following ${targetUserDbId}, no record deleted.`);
    }

    // キャッシュ再検証
    const targetUser = await prisma.user.findUnique({ where: { id: targetUserDbId }, select: { username: true } });
    const loggedInUser = await prisma.user.findUnique({ where: { id: loggedInUserDbId }, select: { username: true } });
    if (targetUser?.username) revalidatePath(`/profile/${targetUser.username}`);
    if (loggedInUser?.username) revalidatePath(`/profile/${loggedInUser.username}`);
    if (loggedInUser?.username) revalidatePath(`/follows/${loggedInUser.username}`); // 自分のフォローリスト

    return { success: true, status: "not_following" };

  } catch (error) {
    console.error(`[FollowActions/Unfollow] Error unfollowing user ${targetUserDbId}:`, error);
    const message = error instanceof Error ? error.message : "フォロー解除中にエラーが発生しました。";
    return { success: false, error: message, status: "error" };
  }
}
...
export async function cancelFollowRequestAction(targetUserDbId: string): Promise<FollowActionResult> {
   const { userId: loggedInClerkId } = await auth();
   if (!loggedInClerkId) { return { success: false, error: "ログインしてください。", status: "error" }; }
   const loggedInUserDbId = await getUserDbIdByClerkId(loggedInClerkId);
   if (!loggedInUserDbId) { return { success: false, error: "ユーザー情報が見つかりません。", status: "error" }; }

   console.log(`[FollowActions/CancelRequest] User ${loggedInUserDbId} attempting to cancel request to ${targetUserDbId}`);
   ...
   try {
     ...
     if (deleteResult.count > 0) {
       console.log(`[FollowActions/CancelRequest] User ${loggedInUserDbId} successfully cancelled request to ${targetUserDbId}`);
     } else {
       // リクエストが存在しなかった、または既に処理されていた場合もエラーとはしない
       console.log(`[FollowActions/CancelRequest] No pending request found from ${loggedInUserDbId} to ${targetUserDbId}.`);
     }
     ...
     console.error(`[FollowActions/CancelRequest] Error cancelling follow request to ${targetUserDbId}:`, error);
     const message = error instanceof Error ? error.message : "フォローリクエストの取り消し中にエラーが発生しました。";
     return { success: false, error: message, status: "error" };
   }
}