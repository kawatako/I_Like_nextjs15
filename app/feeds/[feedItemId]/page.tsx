...
export default async function FeedDetailPage({ params }: FeedDetailPageProps) {
  ...
  const renderCard = () => {
    switch (feedItem.type) {
      ...
      case FeedType.RANKING_UPDATE:
        return <RankingUpdateCard item={feedItem} loggedInUserDbId={loggedInUserDbId} />;
      case FeedType.RETWEET:
        ...
        return <RetweetCard item={feedItem} loggedInUserDbId={loggedInUserDbId} />; // 仮
      case FeedType.QUOTE_RETWEET:
        return <QuoteRetweetCard item={feedItem} loggedInUserDbId={loggedInUserDbId} />;
      default:
  ...
  return (
    ...
    <div className="max-w-xl mx-auto"> {/* 例えば中央に幅を制限 */}
      {/* TODO: 「戻る」ボタンなどを追加 */}
